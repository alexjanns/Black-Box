...
public class Analyser {
		...
		while(keepGoing) {
			...
			if(input.contains(new Coordinates(boardDim-iteration, boardDim*2+1+iteration))) {
				...
				if(marbleCount > 0){
					...
					for(int j = boardDim-1; j >= 0; j--) {
						if(input.contains(new Coordinates(boardDim+1+j, boardDim*2+1+iteration))){
							Coordinates toAdd = new Coordinates(boardDim-iteration-2, j-1); //Array Coordinates
							if(!fixedPos.contains(toAdd)) {
								fixedPos.add(toAdd);
								marbleCount--;
				...
				if(marbleCount > 0){
					...
					for(int j = boardDim-1; j >= 0; j--) {
						if(input.contains(new Coordinates(boardDim*2+1+j, boardDim*3+1+iteration))){
							Coordinates toAdd = new Coordinates(boardDim-j, boardDim-iteration-2); //Array Coordinates
							if(!fixedPos.contains(toAdd)) {
								fixedPos.add(toAdd);
								marbleCount--;
							}
	...
	//Do any of the two positions of the XORMarble to be tested overlap any positions of an existing XORMarble?
	private boolean checkXORforOverlap(XORMarble m) {
